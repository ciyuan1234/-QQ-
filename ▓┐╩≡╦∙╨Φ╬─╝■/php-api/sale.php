...
$type=mysql_real_escape_string($jsondata->type);
$quantity=intval($jsondata->quantity);
...
$coin=intval($row["unitprice"])*intval($quantity);

addCoinHistory($openid,$coin,"售出".$row["type_cn"]."x".$quantity);

exitJson(0,"售出成功",array("coin"=>$coin));
